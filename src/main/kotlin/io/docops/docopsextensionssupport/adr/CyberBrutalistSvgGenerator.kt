 ...
 */
class CyberBrutalistAdrSvgGenerator {
    companion object {
...
<svg xmlns="http://www.w3.org/2000/svg" id="cb_adr_$id" width="$svgWidth" height="$totalHeight" viewBox="0 0 $svgWidth $totalHeight" preserveAspectRatio='xMidYMid meet'>
  <defs>
    <pattern id="hexagons_$id" width="50" height="43.4" patternUnits="userSpaceOnUse" patternTransform="scale(2)">
      <path d="M25 0 L50 14.4 L50 43.4 L25 57.8 L0 43.4 L0 14.4 Z" fill="none" stroke="#1e293b" stroke-width="1" />
    </pattern>
    <style type="text/css">
      @import url('https://fonts.googleapis.com/css2?family=Syne:wght@800&amp;family=JetBrains+Mono:wght@400;700&amp;display=swap');
      
      #cb_adr_$id .canvas { fill: #020617; }
      #cb_adr_$id .bg-pattern { fill: url(#hexagons_$id); }
      #cb_adr_$id .glass-panel { fill: rgba(15, 23, 42, 0.9); stroke: $statusColor; stroke-width: 1.5; }
      #cb_adr_$id .impact-panel { fill: rgba(30, 41, 59, 0.5); stroke: #475569; stroke-width: 1; stroke-dasharray: 4; }
      #cb_adr_$id .accent-line { stroke: $statusColor; stroke-width: 4; stroke-linecap: square; filter: drop-shadow(0 0 5px $statusColor); }
      
      #cb_adr_$id .adr-title { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 18px; fill: #fff; text-transform: uppercase; letter-spacing: -1px; }
      ...
      #cb_adr_$id .mono-text { font-family: 'JetBrains Mono', monospace; font-size: 13px; fill: #94a3b8; }
      ...
      #cb_adr_$id .participant-node:hover .node-name { fill: #fff; }
  ...
  <rect width="100%" height="100%" class="bg-pattern" opacity="0.4" />

  <!-- Header: Moved Status to avoid overlap with wide Syne font -->
  <g class="entry" style="animation-delay: 0s;">
    <line x1="40" y1="60" x2="40" y2="125" class="accent-line" />
    <text x="60" y="90" class="adr-title">${escapeXml(adr.title)}</text>
    
    <!-- Info Row -->
    <text x="60" y="118" class="mono-text">DATE: ${escapeXml(adr.date)} // REF: ADR-${id.take(4).uppercase()}</text>
    
    <!-- Status Badge: Positioned at the far right -->
    <g transform="translate(${svgWidth - 160}, 100)">
  ...
  <g class="entry" style="animation-delay: 0.1s;">
    ...
    ${renderLines(adr.context, 60, (topPanelY + 70).toInt(), 42)}
    ...
    ${renderLines(adr.decision, 450, (topPanelY + 70).toInt(), 42, "#fff")}
  </g>
  ...
  !-- 05: References - Correctly positioned and dynamically sized -->
  <g transform="translate(0, $refPanelY)">
    <g class="entry" style="animation-delay: 0.4s;">
      <rect x="40" y="0" width="760" height="$refPanelHeight" rx="4" fill="rgba(99, 102, 241, 0.05)" stroke="rgba(99, 102, 241, 0.2)" />
...
""".trimIndent()
    ...
    private fun renderParticipants(participants: List<Participant>): String {
        return participants.mapIndexed { i, p ->
            ...
            sb.append("""<circle cx="30" cy="30" r="30" fill="#1e293b" stroke="#6366f1" stroke-width="2" class="node-circle" />""")
            sb.append("""<text x="30" y="38" text-anchor="middle" font-size="24">${if (p.emoji.isEmpty()) "👤" else p.emoji}</text>""")

            var textY = 85
            nameLines.forEach { line ->
                sb.append("""<text x="30" y="$textY" text-anchor="middle" class="mono-text node-name" style="fill:#fff">${escapeXml(line)}</text>""")
                textY += 15
            }
