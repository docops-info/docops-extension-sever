...
class TreemapGenerator(private val useDark: Boolean = false) {
    ...
    private fun generateModernTheme(treemap: Treemap, paletteType: ColorPaletteFactory.PaletteType): String {
        ...
        sb.append("""
            <svg width="${treemap.display.width}" height="${treemap.display.height}" 
                ...
                <defs>
                    <filter id="bg-blur-a-${guid}" x="-50%" y="-50%" width="200%" height="200%">
                        <feGaussianBlur stdDeviation="40"/>
                    </filter>
                    <filter id="bg-blur-b-${guid}" x="-50%" y="-50%" width="200%" height="200%">
                    ...
                    <style type="text/css">
                        ...
                        #ID_${guid} .tile-group:focus-visible .treemap-rect,
                        #ID_${guid} .tile-group:focus-within .treemap-rect {
                            filter: brightness(1.2);
                            outline: none;
                        }
            ...
            "#ID_${guid} #tile-${guid}-${i}:hover ~ .tooltips-layer #tooltip-${guid}-${i} .tooltip-content, " +
                    "#ID_${guid} #tile-${guid}-${i}:focus-within ~ .tooltips-layer #tooltip-${guid}-${i} .tooltip-content " +
        ...
        sb.append("""
            ...
            <circle cx="${treemap.display.width * 0.75}" cy="${treemap.display.height * 0.25}" 
                    r="400" fill="$glowColor1" opacity="0.08" filter="url(#bg-blur-a-${guid})"/>
            <circle cx="${treemap.display.width * 0.25}" cy="${treemap.display.height * 0.75}" 
                    r="350" fill="$glowColor2" opacity="0.06" filter="url(#bg-blur-b-${guid})"/>
        """.trimIndent())
        ...
        rects.forEachIndexed { index, rect ->
            ...
            sb.append(
                """<g class="tile-group" id="tile-${guid}-${index}" tabindex="0" role="img" aria-label="${escapeXml(buildTooltip(item, percentage))}">"""
            )
    ...
    private fun buildCustomTooltip(
        ...
        val rawRectCenterX = rect.x + rect.width / 2
        val minX = (tooltipWidth / 2.0) + 12.0
        val maxX = display.width - (tooltipWidth / 2.0) - 12.0
        val rectCenterX = rawRectCenterX.coerceIn(minX, maxX)

        val spaceAbove = rect.y - 140
        val spaceBelow = display.height - (rect.y + rect.height) - 80

        val useTopTooltip = spaceAbove > (tooltipHeight + offset + 20) || spaceBelow < (tooltipHeight + offset + 20)
