...
class RoadmapMaker(val useDark: Boolean) {
    ...
    """.trimIndent())
        ...
        if (feature.priority != Priority.MEDIUM) {
            ...
            <text x="6" y="18" class="priority-badge">${prioritySymbol.escapeXml()}</text>
        """.trimIndent())
        }
        ...
        if (feature.assignee.isNotEmpty()) {
            ...
            <text x="${cardWidth - 20}" y="25" class="assignee-initials">${initials.escapeXml()}</text>
        """.trimIndent())
        }
    ...
    """.trimIndent())
        ...
        card.append("""
        <text x="40" y="${cardHeight - 8}" class="release-date $statusClass">${releaseDateText.escapeXml()}</text>
    """.trimIndent())
    ...
    private fun generateTooltip(feature: RoadmapFeature, categoryName: String): String {
        ...
        <g class="tooltip" opacity="0">
            ...
            <text x="0" y="-25" class="tooltip-text">
                <tspan x="0" dy="0">Priority: ${feature.priority.name.escapeXml()}</tspan>
                <tspan x="0" dy="12">Effort: ${feature.effort.name.escapeXml()}</tspan>
                <tspan x="0" dy="12">Category: ${categoryName.escapeXml()}</tspan>
                <tspan x="0" dy="12">${dependencyText.escapeXml()}</tspan>
            </text>
        </g>
    """.trimIndent()
    }
