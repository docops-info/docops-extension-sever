...
class WordCloudMaker {
    ...
    private fun addWords(wordCloud: WordCloud): String {
        ...
        sortedWords.forEach { word ->
            var fontSize = wordCloud.calcFontSize(word)
            ...
            var wordWidth = word.text.length * fontSize * 0.55 // Reduced from 0.6 for better space utilization
            var wordHeight = fontSize * 1.1 // Reduced from 1.2 for better space utilization
            ...
            // Increased max attempts from 200 to 500
            ...
            // Fallback strategy: If word can't be placed after many attempts, reduce font size and try again
            if (!placed) {
                // Reduce font size by 30% but ensure it's not smaller than minFontSize
                fontSize = Math.max(wordCloud.display.minFontSize, (fontSize * 0.7).toInt())

                // Recalculate word dimensions with reduced font size
                wordWidth = word.text.length * fontSize * 0.55
                wordHeight = fontSize * 1.1
                ...
                // Try again with reduced font size
                ...
                // Guaranteed placement: If still can't place, force placement at center with minimum size
                if (!placed) {
                    fontSize = wordCloud.display.minFontSize
                    wordWidth = word.text.length * fontSize * 0.55
                    wordHeight = fontSize * 1.1

                    // Find a position that minimizes overlap
                    var bestOverlapCount = Int.MAX_VALUE
                    var bestX = centerX.toDouble()
                    var bestY = centerY.toDouble()

                    // Try different positions in a grid pattern
                    for (gridX in 0..wordCloud.calcWidth() step 50) {
                        for (gridY in 50..wordCloud.calcHeight() step 50) {
                            val testWord = PlacedWord(gridX.toDouble(), gridY.toDouble(), wordWidth, wordHeight)
                            val overlapCount = placedWords.count { it.overlaps(testWord) }

                            if (overlapCount < bestOverlapCount &&
                                gridX - wordWidth/2 > 0 && 
                                gridX + wordWidth/2 < wordCloud.calcWidth() && 
                                gridY - wordHeight/2 > 50 && 
                                gridY + wordHeight/2 < wordCloud.calcHeight()) {
                                bestOverlapCount = overlapCount
                                bestX = gridX.toDouble()
                                bestY = gridY.toDouble()
                            }
                        }
                    }
            ...
            // All words are guaranteed to be placed now
    ...
    private data class PlacedWord(val x: Double, val y: Double, val width: Double, val height: Double) {
        fun overlaps(other: PlacedWord): Boolean {
            // Reduced padding factor (0.9 instead of 1.0) for better space utilization
            // This allows words to be placed closer together
            val paddingFactor = 0.9

            return !(x + (width/2 * paddingFactor) < other.x - (other.width/2 * paddingFactor) ||
                    x - (width/2 * paddingFactor) > other.x + (other.width/2 * paddingFactor) ||
                    y + (height/2 * paddingFactor) < other.y - (other.height/2 * paddingFactor) ||
                    y - (height/2 * paddingFactor) > other.y + (other.height/2 * paddingFactor))
        }
    }
