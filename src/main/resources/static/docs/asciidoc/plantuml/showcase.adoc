...
class ThemeToggleTop {
    ...
    createToggleButton() {
        ...
        button.innerHTML = `
            <span class="light-icon" style="display: var(--light-display)">☀️</span>
            <span class="dark-icon" style="display: var(--dark-display)">🌙</span>
        `;
    ...
    updateTheme() {
        ...
        root.style.setProperty('--light-display', 'none');
        root.style.setProperty('--dark-display', 'none');

        switch (theme) {
            case 'dark':
                root.classList.add('dark-mode');
                root.style.setProperty('--light-display', 'block');
                break;
            case 'auto':
                ...
                if(isDark) {
                    ...
                    root.style.setProperty('--dark-display', 'block');
                }
                break;
            case 'light':
                root.style.setProperty('--dark-display', 'block');
...
@startuml
scale 0.5
!if %variable_exists("$DARK") && ($DARK == "true")
  !$DARK = "true"
!endif
class Animal {
  + eat()
...
skinparam actor {
    BorderColor Black
    BackgroundColor<<author>> #FF9500
    BackgroundColor<<assignee>> #007AFF
}

actor "Manager" <<author>> as pm
actor "Developer" <<assignee>> as Developer

file "ToDo" <<ticket>> as todo #FF3B30
...
rectangle "ID Number" <<property>> as Id
...
+
====
[plantuml,comp100,format=svg,role="popup-diagram"]
----
@startuml
