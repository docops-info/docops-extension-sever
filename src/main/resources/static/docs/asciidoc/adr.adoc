...
+
====
[docops,adr,role=center,controls="false",,scale="0.7"]
----
template=apple
visualVersion=2
...
+
====
[docops,adr,controls=true,,scale="0.7"]
----
theme=pro
template=apple
visualVersion=2
title=Use Event-Driven Integration for Billing Updates
status=Accepted
date=2026-06-07

context
Billing updates currently depend on synchronous calls between services.
This creates retry ambiguity, higher coupling, and cascading failure risk.
Current integration requires direct availability.
Failures are difficult to replay.
Audit history is incomplete.

decision
Adopt asynchronous domain events for billing state changes.
Services publish immutable billing events to the platform event bus.
Downstream consumers subscribe independently.

consequences
Lower coupling between bounded contexts.
Better replay and audit capability.
Eventual consistency becomes explicit.
Consumers must handle duplicate delivery.

participants
Maya Chen | Platform Architecture | maya@example.com | #34C759 | 🧭
Ravi Singh | Billing Engineering | ravi@example.com | #0A84FF | ⚙️
Elena Torres | SRE | elena@example.com | #FF9500 | 🛡️

references
[[https://adr.github.io Architecture Decision Records]]
[[https://internal.wiki/platform/events Event-driven architecture guidance]]
----
====
Rejected::
+
...
+
====
[docops,adr,useDark=true,role=center,controls="false",scale="0.7"]
----
template=brutalist
theme=aurora
---
...
[source,asciidoc]
....
[docops,adr,useDark=true,role=center,scale="0.7"]
----
theme=aurora
template=brutalist
---
...
+
====
[docops,adr,role=center,scale="0.7"]
----
theme: reo
visualVersion=2
---
