...
* Responsive design with scaling options
...
|headerTitle
|The title for the header section

|scale
|A scaling factor for the SVG (default: 1.0)
...
|headerColor
|Header background color (default: #8e44ad)
...
* `!` - Exclamation mark for critical items
* `$` - Dollar sign for cost-related items
* `✓` - Check mark for good items
...
* Scaling for different display sizes
* Custom icons and status indicators
* Flexible layout for different metrics and optimizations
